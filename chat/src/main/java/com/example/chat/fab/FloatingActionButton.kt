...
open class FloatingActionButton : ImageButton {
    ...
    private var mSize: FabSize = FabSize.Normal
    ...
    private var iconDrawable: Drawable
        get() = mIconDrawable?.let {
            it
        } ?: let {
            if (mIcon != 0) {
                resources.getDrawable(mIcon, null)
            } else {
                ColorDrawable(Color.TRANSPARENT)
            }
        }
        set(@NonNull iconDrawable) {
    ...
    internal open fun init(context: Context, attributeSet: AttributeSet?) {
    ...
    private fun updateCircleSize() {
        mCircleSize = getDimension(if (mSize === FabSize.Normal) R.dimen.fab_size_normal else R.dimen.fab_size_mini)
    }
    ...
    private fun getDimension(@DimenRes id: Int): Float {
    ...
    internal open fun updateBackground() {
        ...
        val layerDrawable = LayerDrawable(
                arrayOf(resources.getDrawable(if (mSize === FabSize.Normal) R.drawable.fab_bg_normal else R.drawable.fab_bg_mini, null), createFillDrawable(strokeWidth), createOuterStrokeDrawable(strokeWidth), iconDrawable))
    ...
    private fun createFillDrawable(strokeWidth: Float): StateListDrawable {
        ...
        drawable.addState(intArrayOf(-android.R.attr.state_enabled), createCircleDrawable(mColorDisabled, strokeWidth))
        drawable.addState(intArrayOf(android.R.attr.state_pressed), createCircleDrawable(mColorPressed, strokeWidth))
    ...
    private class TranslucentLayerDrawable(private val mAlpha: Int, vararg layers: Drawable) : LayerDrawable(layers) {
        ...
        override fun draw(canvas: Canvas) {
            val bounds = bounds
            canvas.saveLayerAlpha(bounds.left.toFloat(), bounds.top.toFloat(), bounds.right.toFloat(), bounds.bottom.toFloat(), mAlpha)
            super.draw(canvas)
            canvas.restore()
        }
    }
    ...
    private fun createInnerStrokesDrawable(color: Int, strokeWidth: Float): Drawable {
        ...
        shapeDrawable.shaderFactory = object : ShaderFactory() {
            override fun resize(width: Int, height: Int): Shader {
                return LinearGradient((width / 2).toFloat(), 0f, (width / 2).toFloat(), height.toFloat(),
    ...
    sealed class FabSize(val ordinal: Int) {
        object Normal: FabSize(0)
        object Mini: FabSize(1)

        companion object {
            fun getSizeForOrdinal(ordinal: Int): FabSize = when (ordinal) {
                FabSize.Normal.ordinal -> FabSize.Normal
                FabSize.Mini.ordinal -> FabSize.Mini
                else -> FabSize.Normal
            }
        }
    }
